...
from file_management.models import LoginEvent
from django.contrib import messages
from django.shortcuts import redirect
...
def logout(request):
    if request.user.is_authenticated:
        # Retrieve the last login event with a null log_out_time for the user
        login_event = LoginEvent.objects.filter(user=request.user, log_out_time__isnull=True).order_by(
            '-login_time').first()
        if login_event:
