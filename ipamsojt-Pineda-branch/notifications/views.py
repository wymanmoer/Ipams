...
class AdviserNotification(View):
    ...
    def get(self, request):
        user = request.user
        notifications = Notification.objects.filter(
            Q(recipient=user.id) | Q(notif_type=NotificationType.objects.get(pk=6)) | Q(
                notif_type=NotificationType.objects.get(pk=1)) | Q(
                notif_type=NotificationType.objects.get(pk=2))).order_by('-date_created')
        request.session['notif_count'] = notifications.count()
        ...
        if request.method == 'POST':
            ...
            if request.POST.get('displayContent'):
                ...
                notifications = Notification.objects.filter(
                    Q(recipient=user.id) | Q(notif_type=NotificationType.objects.get(pk=6)) | Q(
                        notif_type=NotificationType.objects.get(pk=1)) | Q(
                        notif_type=NotificationType.objects.get(pk=2)))
...
class KTTONotification(View):
    ...
    def post(self, request):
        if request.method == 'POST':
            ...
            # for filter
            if 'all' in request.POST:
                notifications = Notification.objects.filter(Q(to_ktto=True) | Q(recipient=user.id)).order_by(
                    '-date_created')
                return render(request, self.name, filterHelper(request, notifications, 'ALL'))
            elif 'read' in request.POST:
                notifications = Notification.objects.filter(
                    (Q(to_ktto=True) | Q(recipient=user.id)) & Q(is_read=True)).order_by('-date_created')
                return render(request, self.name, filterHelper(request, notifications, 'Read'))
            elif 'unread' in request.POST:
                notifications = Notification.objects.filter(
                    (Q(to_ktto=True) | Q(recipient=user.id)) & Q(is_read=False)).order_by('-date_created')
                return render(request, self.name, filterHelper(request, notifications, 'Unread'))
            elif 'rrs' in request.POST:
                notifications = Notification.objects.filter((Q(to_ktto=True) | Q(recipient=user.id)) & Q(
                    notif_type=NotificationType.objects.get(pk=1))).order_by('-date_created')
                return render(request, self.name, filterHelper(request, notifications, 'Role Request Student'))
            elif 'rra' in request.POST:
                notifications = Notification.objects.filter((Q(to_ktto=True) | Q(recipient=user.id)) & Q(
                    notif_type=NotificationType.objects.get(pk=2))).order_by('-date_created')
                ...
                notifications = Notification.objects.filter((Q(to_ktto=True) | Q(recipient=user.id)) & Q(
                    notif_type=NotificationType.objects.get(pk=3))).order_by('-date_created')
                return render(request, self.name, filterHelper(request, notifications, 'New Record Proposal/Thesis'))
            elif 'nrp' in request.POST:
                notifications = Notification.objects.filter((Q(to_ktto=True) | Q(recipient=user.id)) & Q(
                    notif_type=NotificationType.objects.get(pk=4))).order_by('-date_created')
                return render(request, self.name, filterHelper(request, notifications, 'New Record Project'))
            elif 'resubmission' in request.POST:
                notifications = Notification.objects.filter((Q(to_ktto=True) | Q(recipient=user.id)) & Q(
                    notif_type=NotificationType.objects.get(pk=5))).order_by('-date_created')
                return render(request, self.name, filterHelper(request, notifications, 'Resubmissions'))
...
class RDCONotification(View):
    ...
    def post(self, request):
        if request.method == 'POST':
            ...
            # for filter
            if 'all' in request.POST:
                notifications = Notification.objects.filter(Q(to_rdco=True) | Q(recipient=user.id)).order_by(
                    '-date_created')
                return render(request, self.name, filterHelper(request, notifications, 'ALL'))
            elif 'read' in request.POST:
                notifications = Notification.objects.filter(
                    (Q(to_rdco=True) | Q(recipient=user.id)) & Q(is_read=True)).order_by('-date_created')
                return render(request, self.name, filterHelper(request, notifications, 'Read'))
            elif 'unread' in request.POST:
                notifications = Notification.objects.filter(
                    (Q(to_rdco=True) | Q(recipient=user.id)) & Q(is_read=False)).order_by('-date_created')
                return render(request, self.name, filterHelper(request, notifications, 'Unread'))
            elif 'rrs' in request.POST:
                notifications = Notification.objects.filter((Q(to_rdco=True) | Q(recipient=user.id)) & Q(
                    notif_type=NotificationType.objects.get(pk=1))).order_by('-date_created')
                return render(request, self.name, filterHelper(request, notifications, 'Role Request Student'))
            elif 'rra' in request.POST:
                notifications = Notification.objects.filter((Q(to_rdco=True) | Q(recipient=user.id)) & Q(
            ...
            if request.POST.get('displayContent'):
                ...
                return notificationContent(request, id, notifications)
                # notif = Notification.objects.get(pk=id)
                # if notif.notif_type.name == 'Request to Download Abstract':
                #     record_status = CheckedRecord.objects.filter(record=notif.record_id).latest('status')
                #     return JsonResponse({'success': True,
                #                          'title': notif.record.title,
                #                          'classification': notif.record.classification.name,
                ...
                #                          'authors': listOfAuthors(request, notif.record.id),
                #                          'status': record_status.status,
                #                          'subject_body': notif.notif_type.name + ' by ' + notif.user.first_name +
                #                                          notif.user.last_name,
                #                          'checked_by': notif.user.first_name + ' ' + notif.user.last_name,
                #                          })
